...
- Computes mIoU.
- Saves colorized masks + overlay images.
...
class TinySegNet(nn.Module):
    def __init__(self, num_classes=NUM_CLASSES):
        ...
        self.dec_mid = nn.Sequential(nn.Conv2d(64,64,3,padding=1,bias=False), nn.BatchNorm2d(64), nn.SiLU())
        self.dec_low = nn.Sequential(nn.Conv2d(32,64,3,padding=1,bias=False), nn.BatchNorm2d(64), nn.SiLU())
...
# Dataset + transforms
# -----------------------
def normalize(t): return TF.normalize(t, IMAGENET_MEAN, IMAGENET_STD)
...
def overlay_pred_on_image(pil_img: Image.Image, pred_np: np.ndarray, alpha: float = 0.5):
...
def main():
    ...
    print("Per-class IoU:", [round(float(x), 4) for x in per_cls])
    ...
    with torch.no_grad():
        for batch in val_loader:
            ...
            for i in range(preds.shape[0]):
                mask_img = colorize_mask(preds[i])
                overlay = overlay_pred_on_image(pil_imgs[i], preds[i])
            ...
            if saved >= args.save_max:
                break
    print(f"[i] Saved {saved} results -> {outdir}")
